jQuery(document).ready(function () {
    ...
    function verifier_land_init() {
                ...
                template: function (row) {
                    ...
                    if (row.approvalStatus == 0) {
                        return '<div class="row action_btn_row">\
                                ...
                                <a data-id="'+ row._id + '" data-userid="' + row.user_id + '" data-certificateno="' + row.certificate_no + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="approve_land_record" title="Approve Land Record">\
                                    <i class="la la-check"></i>\
                                </a>\
                            </div>\
                    ';
                    } else if (row.approvalStatus == 2 || row.approvalStatus == 3) {
                        return '<div class="row action_btn_row">\
                        <a data-id="'+ row._id + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="view_full_details" title="View Full Details">\
                            <i class="la la-eye"></i>\
                        </a>\
                        <a data-id="'+ row._id + '" data-comment="' + row.comment + '" data-status="' + row.approvalStatus + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="show_comment" title="Show Comment">\
        ...
        if (approvalStatus != 2) {
            datatable.hideColumn('rejected_on')
        }

        $("#show_land_records_verifier").on("click", 'tbody #approve_land_record', function (e) {
