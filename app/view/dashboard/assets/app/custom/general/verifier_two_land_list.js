    ...
    function verifier_land_init() {
        ...
        var datatable = $('#show_land_records_verifier').KTDatatable({
            ...
            data: {
                ...
                source: {
                    read: {
                        url: domain + '/listVerifierLandRecords?region_id=' + user.region_id + '&approvalStatus=' + approvalStatus + '&hash=' + hash,
                        method: 'GET'
                    },
                },
                pageSize: 10,
            ...
            },
            {
                ...
                textAlign: 'center',
                template: function (row) {
                    var filehash1, filename1, filehash2, filename2
                    ...
                    return '<div class="row action_btn_row">\
                                <a data-id="'+ row._id + '" data-comment="' + row.comment + '" data-comment2="' + row.comment2 + '" data-comment3="' + row.comment3 + '" data-comment4="' + row.comment4 + '" data-fileHash1="' + filehash1 + '" data-fileName1="' + filename1 + '" data-fileHash2="' + filehash2 + '" data-fileName2="' + filename2 + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="view_verified_question" title="View Verified Details">\
                ...
                template: function (row) {
                    if (row.approvalStatus == 7) {
                        return '<div class="row action_btn_row">\
                                ...
                                <a data-id="'+ row._id + '" data-userid="' + row.user_id + '" data-certificateno="' + row.certificate_no + '" data-certificatetype="' + row.certificate_type + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="approve_details" title="Approve Records">\
                                    <i class="la la-check"></i>\
                                </a>\
                            </div>\
                    ';
                    } else if (row.approvalStatus == 2 || row.approvalStatus == 4) {
                        return '<div class="row action_btn_row">\
                        <a data-id="'+ row._id + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="view_full_details" title="View Full Details">\
                            <i class="la la-eye"></i>\
                        </a>\
                        <a data-id="'+ row._id + '" data-comment="' + row.comment2 + '" data-status="' + row.approvalStatus + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="show_comment" title="Show Comment">\
        ...
        $("#show_land_records_verifier").on("click", 'tbody #approve_details', function (e) {
            ...
            location.href = 'verifier_two_checklist.html?id=' + row + '&user=' + $(this).data('userid') + '&certi=' + $(this).data('certificateno') + '&certiType=' + $(this).data('certificatetype')
        })
        ...
        $("#show_land_records_verifier").on("click", 'tbody #view_verified_question', function (e) {
            ...
            var comment2 = $(this).data('comment2');
            var comment3 = $(this).data('comment3');
            var comment4 = $(this).data('comment4');
            var html = ''
            console.log(comment, comment2, comment3, comment4)
