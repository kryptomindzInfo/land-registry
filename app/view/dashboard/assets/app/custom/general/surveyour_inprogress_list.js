jQuery(document).ready(function () {
    ...
    function verifier_land_init() {
        ...
        var datatable = $('#show_land_records_verifier').KTDatatable({
            ...
            },
            {
                ...
                template: function (row) {
                    ...
                    return '<div class="row action_btn_row">\
                                <a data-id="'+ row._id + '" data-comment="' + row.land_details.comment + '"  data-comment2="' + row.land_details.comment2 + '" data-fileHash1="' + filehash1 + '" data-fileName1="' + filename1 + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="view_verified_question" title="View Verified Details">\
            ...
            {
                field: 'show_changed_details',
                title: 'Show Changed Details',
                width: 100,
                sortable: false,
                autoHide: false,
                overflow: 'visible',
                ...
                template: function (row) {
                    return '<div class="row action_btn_row">\
                                <a data-id="'+ row.land_details._id + '" data-comment="' + row.land_details.comment + '"  data-comment2="' + row.land_details.comment2 + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="show_survey_changed" title="Show Changed Details">\
                                    <i class="la la-book"></i>\
                                </a>\
                            </div>\
                    ';
            ...
            },
            {
                ...
                template: function (row) {
                    ...
                    return '<div class="row action_btn_row">\
                                ...
                                <a  data-id="'+ row.land_id + '" data-land=' + JSON.stringify(row) + ' data-userid="' + row.land_user + '" data-certificateno="' + row.land_details.certificate_no + '" class="btn btn-clean btn-icon btn-sm btn-icon-md"  id="edit_draft_application" title="Edit Details">\
        ...
        $("#show_land_records_verifier").on("click", 'tbody #edit_draft_application', function (e) {
            ...
            location.href = 'surveyour_checklist.html?id=' + row + '&user=' + $(this).data('userid') + '&certi=' + $(this).data('certificateno')
        })

        $("#show_land_records_verifier").on("click", 'tbody #show_survey_changed', function (e) {
            //e.preventDefault()
            var row = $(this).data('id');
            $.when($.get('/getSingleChecklist?verifier_id=' + user._id + '&application_id=' + row)).done(function (_res) {

                var data = _res.data[0]

                $("#show_survey_changed_modal").modal('toggle')
                $("#pnorth").val(data.parcel_data.north)
                $("#psouth").val(data.parcel_data.south)
                $("#peast").val(data.parcel_data.east)
                $("#pwest").val(data.parcel_data.west)
                $('#array_lat_lng_show').val(JSON.stringify(data.draw_data.land_markers)).trigger("change")
