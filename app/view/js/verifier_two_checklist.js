jQuery(document).ready(function () {
  ...
  var certificate_type = urls.searchParams.get("certiType");
  ...
  if (certificate_type == 2) {
    $("#submit_btn").html("Generate Certificate");
  }
  ...
  $.when($.get("/getSingalLand?id=" + application_id)).done(function (_res) {
    ...
    console.log(land);
    if (
      land.approvalStatus == 4 ||
      land.approvalStatus == 2 ||
      land.approvalStatus == 1
    ) {
      hideButtons();
    } else {
      showButtons();
  ...
  $("#checklist_form").on("submit", function (e) {
    ...
    if (questions_check == globals.VERIFIER_TWO_QUESTIONS) {
  ...
  $("#save_draft_btn").click(function () {
    showgenProgress();
    var certificate_no_que = $("#certificate_no_question").prop("checked");
    var land_details_que = $("#land_details_question").prop("checked");
  ...
  $("#comment_forward_form").on("submit", function (e) {
    e.preventDefault();
    showgenProgress();
    let options = {};
    options.application_id = application_id;
    options.user_id = user_id;
    options.verifier_id = user._id;
    options.surveyour_id = "surveyor";
    options.comment = $("#forward_comment_text").val();
    $.when($.post("/forwardLandRecord", options)).done(function (_res) {
      hidegenProgress();
      if (_res.code == 200) {
        PNotify.success({
          title: "Success!",
          text: "Application Sent for Survey Successfully!",
        });
        // window.history.back()
        location.href = "../index.html";
      } else {
  ...
  $("#comment_submit_form").on("submit", function (e) {
    ...
    options.certificateType = certificate_type;
    options.user_id = user_id;
    options.verifier_id = user._id;
    if ($("#file_hash").val() != "") {
      options.extraDocs = {
    ...
    $.when($.post("/submitChecklist", options)).done(function (_res) {
      ...
      if (_res.code == 200) {
        ...
        var status = _res.approvalStatus;
        hideButtons();
