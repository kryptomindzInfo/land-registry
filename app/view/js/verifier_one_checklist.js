jQuery(document).ready(function () {
  ...
  $.when($.get("/getSingalLand?id=" + application_id)).done(function (_res) {
    ...
    console.log(land);
    if (land.approvalStatus == 6 || land.approvalStatus == 2) {
      hideButtons();
    } else {
      showButtons();
      $.when(
  ...
  $("#checklist_form").on("submit", function (e) {
    ...
    if (questions_check == globals.VERIFIER_ONE_QUESTIONS) {
  ...
  $("#filetype").change(function () {
    var action =
      "/addLandRecords?type=" +
      $(this).val() +
      "&class=file_hash&name=file_name&id=v1";
  ...
  $("#comment_submit_form").on("submit", function (e) {
    ...
    options.status = 6;
    options.user_id = user_id;
    options.send_verifier_id = "surveyor";
    options.verifier_id = user._id;
    if ($("#file_hash").val() != "") {
      options.extraDocs1 = {
